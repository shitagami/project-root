...
function getUVCategory(uvIndex) {
    ...
    if (uvIndex <= 2) {
        ...
        return { category: '中程度', imagePath: 'images/Figma_basics_22.png', advice: advice };
    } else if (uvIndex <= 7) {
        advice = '帽子や日傘、長袖の服で肌をしっかり守りましょう。';
        return { category: '強い', imagePath: 'images/Figma_basics_23.png', advice: advice };
    } else if (uvIndex <= 10) {
        advice = 'なるべく外出を控え、紫外線から肌を守るために帽子や日傘を使用しましょう。';
        return { category: '非常に強い', imagePath: 'images/Figma_basics_24.png', advice: advice };
    } else {
        advice = '極力外出を避け、しっかりと紫外線対策を行いましょう。';
        return { category: '極端に強い', imagePath: 'images/Figma_basics_25.png', advice: advice };
    }
}
...
async function displayCurrentLocationData() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            const districtName = weatherData.city.district || ''; // 地区名を取得（存在する場合）

            // UV指数のカテゴリとアドバイスを表示
            document.getElementById('current-uv-index').innerHTML = 
                `現在地のUV指数: ${uvCategoryData.category}<br>
